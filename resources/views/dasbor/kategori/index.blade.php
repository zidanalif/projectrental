...
<div class="card">
        ...
        <div class="tableBox">
            ...
            <div class="tableLayout">
                <table class="table tableSort" data-dom="ltrip">
                    ...
                    @foreach($tableData as $data)
                        <tr>
							<td class="w-5"><a href="{{route("dasbor.kategori.edit",[$data->id])}}">{{$data->id}}</a></td>
							<td class="nowrap">{{$data->kategori}}</td>
                            <td class="w-5 no-sort"><a href="{{route('dasbor.kategori.edit',[$data->id])}}"><i class="fas fa-edit fa-fw"></i></a></td>
                            @if(Gate::any(['kategori_allow']))
            ...
            <div class="row">
                <div class="col-12 col-sm order-3 order-sm-0 pt-3">
                    @if(Gate::any(['kategori_allow']))
                        <a href="{{route('dasbor.kategori.create')}}" class="btn btn-primary" role="button"><i class="fas fa-plus fa-fw"></i> {{trans('global.table_add')}}</a>
                    @endIf
                </div>
    ...
    <div class="modal fade" id="deleteConfirm" tabindex="-1" role="dialog" aria-labelledby="deleteConfirm" aria-hidden="true">
        <div class="modal-dialog">
            <form method="post" action="{{route("dasbor.kategori.delete")}}">
            @method('DELETE')
            @csrf
